...
async def upload_document(
    ...
    Raises ValueError if the file is not a valid PDF or exceeds size limits.
    """
    # Read file content
    ...
    if not content[:4].startswith(PDF_MAGIC_BYTES):
        raise ValueError(
            "Only PDF files are supported. File does not appear to be a valid PDF."
        )
    ...
    try:
        ...
        logger.error(
            "Failed to write file to disk", filename=original_filename, error=str(e)
        )
    ...
    # Create the document record — clean up file on DB failure
    try:
        document = Document(
        ...
        logger.exception("Failed to save document record", filename=original_filename)
        try:
            os.remove(file_path)
        except OSError:
            pass
...
async def delete_document(session: AsyncSession, document_id: str) -> bool:
    """Delete a document and its file from disk. Returns True if it existed."""
    ...
    try:
        if file_path and os.path.exists(file_path):
            os.remove(file_path)
            logger.info(
                "Deleted document file", document_id=document_id, path=file_path
            )
    except OSError:
        logger.exception(
            "Failed to delete document file", document_id=document_id, path=file_path
        )
